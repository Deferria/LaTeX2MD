...
1. 不能识别附有 boolean 参数的环境; **所以不要使用这样的环境!**
2. 在识别环境的时候, 由于正则表达式的限制, 目前只能识别缩进为 2\*4 空格的环境, 并且其内部内容必须缩进为 3\*4 空格. 这一点可能有解决方法, 但是需要更为复杂的匹配机制.
3. **由于不同的渲染器有不同的显示标准, 尽量不要在公式中直接使用特殊字符! 请使用 `ast` 来代替 `*`, 请使用 `ver` 或 `abs` 来代替 `|` ！** 当然公式外还可以使用. 这一点想不到什么好的解决方法.
4. 在 `macroescape.py` 中, 所有的命令都需要人为添加进替换规则中;
5. 原有的 counter 系统并无记录, 也没有保留在 JSON 文件中. 这就导致了链接系统目前只考虑了 `\hyperref` 而没有考虑 `\ref` .
6. 层次系统如 section, subsection 等等, 目前没有被保留在 JSON 文件中, 也没有被写入 Markdown 文件中. 